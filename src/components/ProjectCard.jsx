...
export default function ProjectCard({ image, title, body, techs }) {
    return (
        <div className='flex flex-col md:flex-row mt-10 py-8'>
            <motion.div initial={{ x: -200, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} transition={{ duration: 0.8 }} className='flex w-full md:w-1/2 justify-center'>
                <img className='w-full md:w-1/2 rounded-xl h-[80%]' src={image} alt={title} />
            </motion.div>
            <motion.div initial={{ x: 200, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} transition={{ duration: 0.8 }} className='w-full md:w-1/2 mt-5 md:mt-0'>
